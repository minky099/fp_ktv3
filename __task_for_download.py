                    ...
                    try:
                        ...
                        if entity.data['filename']['is_matched']:
                            if entity.data['meta']['find']:
                                ...
                                if is_dry == False:
                                    SupportFile.file_move(os.path.join(base, original_filename), data['result_folder'], data['result_filename'])
                        else:
                    ...
                    for regex in regex_list:
                        try:
                            if re.search(regex, filename):
                                ...
                                data['result_filename'] = original_filename
                                if is_dry == False:
                                    SupportFile.file_move(os.path.join(base, original_filename), target_folder, original_filename)
                                return
                        except Exception as e: 
                                P.logger.error(f"Exception:{e}")
                                P.logger.error(traceback.format_exc())
